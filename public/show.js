...
(async function () {
  ...
  // Built-in demo checkout (used when the site has no Razorpay keys).
  function openDemoCheckout(order) {
    const dlg = $('demopay');
    const bookingId = hold.bookingId;
    $('demopay-desc').textContent = `${currentShow.title}, seats ${hold.seats.join(', ')}`;
    $('demopay-amount').textContent = rupees(order.amount);
    dlg.querySelector('[data-demo="pay"]').textContent = `Pay ${rupees(order.amount)}`;
    const buttons = dlg.querySelectorAll('[data-demo]');
    ...
    buttons.forEach((b) => (b.onclick = async () => {
      ...
      try {
        const r = await api('/api/payments/demo-pay', { method: 'POST', body: { orderId: order.orderId, outcome: action } });
        dlg.close();
        if (r.failed) return notify('Payment failed. Your seats are still held, so you can try again.');
        sessionStorage.removeItem(HOLD_KEY);
        location.href = `/bookings?highlight=${bookingId}`;
      } catch (e) { dlg.close(); notify(e.message); }
    }));
    dlg.onclose = () => { payBtn.disabled = false; renderBar(); };
    dlg.showModal();
  }
  ...
  payBtn.onclick = async () => {
    ...
    if (typeof Razorpay === 'undefined') { payBtn.disabled = false; renderBar(); return notify('The payment window could not load. Check your internet and refresh the page.'); }
