...
async function showTestBanner() {
  try {
    const { testMode, provider } = await api('/api/config');
    const text = provider === 'demo'
      ? 'Demo site: bookings are real, payments are simulated. No money is charged.'
      : 'Test mode: no real money is charged. At checkout pick UPI and enter success@razorpay, or use a Razorpay test card.';
    if (testMode) document.body.prepend(el('div', { className: 'testbar', textContent: text }));
  } catch {}
}
