...
| `RAZORPAY_KEY_ID` | Optional. Razorpay key id (`rzp_test_...` or `rzp_live_...`) |
| `RAZORPAY_KEY_SECRET` | Optional. Razorpay key secret |
| `RAZORPAY_WEBHOOK_SECRET` | Optional. Secret you set on the Razorpay webhook |
| `ADMIN_PASSWORD` | Password for `/admin`, at least 8 characters |
| `HOLD_MINUTES` | Optional, seat hold length, default 8 |

Without Razorpay keys the app uses a built-in demo gateway (`src/demoGateway.js`) with the same interface, so holds, late-payment handling and refunds behave identically. It allows one successful payment per order, enforced by a unique index.
...
`tests/run-tests.js` starts a stand-in Razorpay API and two app servers sharing one PostgreSQL database, then checks 29 scenarios in Razorpay mode, and `tests/run-demo-tests.js` checks 9 more in demo mode, including: 50 users racing for one seat across both servers; 20 overlapping multi-seat requests; duplicate requests across servers; browser confirmation plus two webhooks arriving together; forged signatures; underpayment; someone else's payment or booking; hold expiry; late payment with and without the seat still free; closed tab after paying; admin validation.
