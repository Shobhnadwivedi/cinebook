...
const rzp = require('./gateway'); // Razorpay, or the demo gateway if no keys are set
...
async function startPayment(user, bookingId) {
  ...
  return { provider: rzp.name, keyId: rzp.KEY_ID, orderId, amount: b.amount, holdExpiresAt: b.hold_expires_at,
...
// Demo mode only: the customer clicked Pay (or Fail) on the built-in checkout.
async function demoPay(user, body) {
  if (rzp.name !== 'demo') throw new BookingError(404, 'Not found.');
  const orderId = String(body.orderId || '');
  const p = (await query(
    'SELECT p.amount, b.user_id FROM payments p JOIN bookings b ON b.id = p.booking_id WHERE p.gateway_order_id = $1', [orderId])).rows[0];
  if (!p || p.user_id !== user.id) throw new BookingError(404, 'Order not found.');
  const paymentId = await rzp.pay(orderId, p.amount, body.outcome);
  if (body.outcome === 'failed') return { failed: true };
  return settle(orderId, paymentId);
}

module.exports = { startPayment, verifyCheckout, handleWebhook, reconcile, settle, demoPay };
