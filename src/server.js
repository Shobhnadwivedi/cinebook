...
app.get('/api/config', (req, res) => res.json({ provider: rzp.name, testMode: !rzp.isLive }));
...
app.post('/api/payments/demo-pay', auth.requireAuth, async (req, res) => {
  res.json(await payments.demoPay(req.user, req.body));
});
...
async function start() {
  ...
  const server = app.listen(PORT, () => console.log(`CineBook running on port ${PORT} (${rzp.name === 'demo' ? 'DEMO' : rzp.isLive ? 'LIVE' : 'TEST'} payments)`));
